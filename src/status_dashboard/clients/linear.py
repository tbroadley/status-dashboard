...
def get_my_issues(api_key: str | None = None) -> list[Issue]:
    """Get active Linear issues assigned to the current user, sorted by status."""
    key = api_key or os.environ.get("LINEAR_API_KEY")
    if not key:
        logger.warning("LINEAR_API_KEY not set, skipping Linear issues")
        return []
    ...
    if "errors" in data:
        logger.error("Linear API error getting assigned issues: %s", data["errors"])  # pyright: ignore[reportAny]
        return []

    data_payload = _get_dict(data, "data")
    viewer = _get_dict(data_payload, "viewer")
    viewer_name = _get_str(viewer, "displayName") or _get_str(viewer, "name") or None
    initials = get_initials(viewer_name)
    ...
    issues = [
        ...
        for issue in _get_list(_get_dict(viewer, "assignedIssues"), "nodes")
    ]
